...
### 6. `commit`（prepare + hook 校验循环）

提交流程简化为**单路径**：`commit prepare` → subagent 审计 → `git commit` → commit-msg hook 校验。

**核心原则**：防护落在变更质量上，不落在谁按了按钮上。commit-msg hook 检查三件事：subagent 审计确实发生、变更计划已推进到可实施阶段、引用的形式化规格完整。

ADR-011 移除了 `commit submit` 命令和 token/TTY 机制，commit-msg hook 直接校验 subagent session log + plan stage + Spec 完整性。

#### 6a. AI 域：`prepare` + 自由语义审计
...
1. 运行 `~/.claude/skills/spec-vc/.venv/bin/spec-vc commit prepare --message "<完整 commit message>"`
   - Spec 就绪检查通过后，写 commit message 到 `.git/spec-vc-commit-msg`
   - **ADR-017**：如果 commit message 含 `[ADR-NNN]` 或 `[ADR-none]`，同时计算并写入 `.git/spec-vc-audit-anchor`（格式 `ADR-XXX@<sha12>`，`sha12` 是 staged diff 内容指纹），stdout 输出 `audit-anchor: <anchor>` 供 AI 复述
   - stdout 输出 staged files + spec 信息
   - 返回非零 = 阻塞（Spec 未就绪 / 无 staged changes）

2. **AI 自由执行语义审计**（无机械分配）：
   - AI 根据需要启动 subagent 进行代码审查、测试验证等
   - **ADR-017 要求**：启动 audit subagent 时，其 `description` 参数**必须包含上一步输出的 anchor 字符串**——这是 commit-msg hook 验证「audit 与本次 staged 内容真实绑定」的唯一证据
   - PostToolUse hook 自动记录所有 Agent 调用到 `.git/spec-vc-subagent-sessions.log`
   - AI 应确保 subagent 审计覆盖所有关键变更

3. **AI 执行 git commit**：审计完成后直接 `git commit`，commit-msg hook 自动校验。

#### 6b. commit-msg hook 校验链

hook 在 `git commit` 时自动触发，校验链：
...
6. **Spec 完整性检查**（[ADR-NNN] 时）：
   ...
   - 无关联 Spec → 放行
7. **audit anchor 绑定检查**（ADR-017，[ADR-NNN] 时）：
   - 读 `.git/spec-vc-audit-anchor`：文件不存在 → 阻塞（要求走 commit prepare 生成 anchor）
   - 读 session log 末行 description：不含 anchor 子串 → 阻塞 + 输出当前 anchor
   - 含 anchor → 放行
   - [ADR-none] → 跳过此检查（豁免规则已量化卡控）
8. 全部通过 → 放行
...
Claude Code 每次 Agent 工具调用后触发，自动记录调用：
...
**ADR-017 收紧**：hook 在 stdin JSON 的 `hook_event_name == "PostToolUseFailure"` 时跳过写日志（hygiene；Agent 业务失败大多仍走 PostToolUse 不命中此分支，但 harness 中断 / 显式失败场景下命中）。同时 commit-msg hook 新增 audit anchor 绑定检查——「audit 真实发生」的证据从「description 非空」升级为「description 含 staged 内容指纹」，让通过门禁的最小成本至少等于读一次 staged diff。
