...
提交分为 **AI 域**（prepare + 语义审计）和 **用户域**（TTY submit）两阶段。

**核心原则**：token 仅在用户 TTY 终端的 `submit` 阶段写入；commit-msg hook 通过 PostToolUse hook 记录的 subagent 调用日志确认真实的审计发生过，AI 无法绕过。

ADR-010 移除了 ADR-005 引入的机械 manifest/audit-report/test-report/verify 层——PostToolUse hook 已在 Harness 层提供不可伪造的审计证据，无需事后机械格式检查。
...
1. 运行 `~/.claude/skills/spec-vc/.venv/bin/spec-vc commit prepare --message "<完整 commit message>"`
   ...
   - 写 prepare 时间戳到 `.git/spec-vc-prepare-ts`
...
3. **AI 在此停止**，向用户输出："审计完成，请在终端运行 `spec-vc commit submit`"
...
2. 交互确认："Press Enter to confirm commit"
3. 写入 basic token（uuid + timestamp 两行）
4. 执行 `git commit -F .git/spec-vc-commit-msg`
5. 删除 `.git/spec-vc-commit-msg`

#### 6c. commit-msg hook（subagent session 检查）

hook 校验链：
1. SPEC_VC_BYPASS 非空 → 完全跳过后续检查（raw escape）
2. 校验 token 存在 + 未过期（300s）
3. 检查 `.git/spec-vc-subagent-sessions.log` 存在且非空
   - 缺失或为空 → 阻塞（"未找到 subagent 审计记录，请确保 PostToolUse hook 已配置"）
4. ADR 引用校验（不变）
5. 全部通过 → 消费 token + 放行

#### 6d. PostToolUse hook（subagent 调用追踪）
...
bypass 在 spec-vc binary 真正损坏时作为 raw escape：设置 `SPEC_VC_BYPASS=<原因>` 后 `git commit` 完全跳过 token + subagent session 检查。ADR 引用校验照常。bypass 触发写入 `.git/spec-vc-bypass.log`。
