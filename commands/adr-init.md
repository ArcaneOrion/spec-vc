---
description: 在当前项目中初始化 spec-vc 基础设施,创建 doc/arch/ 目录并安装 git hooks
---
...
如果用户传 `--no-seed`,跳过此步。后续第一条 ADR 由 `/spec-vc adr-new` 创建,编号仍从 000 起。
...
- 代价:skill 升级后各项目 hooks 漂移,需要显式运行 `/spec-vc adr-upgrade` 同步
- symlink 的替代方案会导致"skill 目录一动所有项目 hooks 全坏",风险过高

**冲突处理**:如果 `.git/hooks/commit-msg` 已存在且非 spec-vc(文件头无 `spec-vc` 字样),询问用户:
- (a) 备份旧 hook 为 `.git/hooks/commit-msg.backup-<timestamp>` 后覆盖
- (b) 中止,由用户手动合并
- (c) 把旧 hook 内容作为第一步,spec-vc 校验作为第二步,串联执行
...
默认 `ADR_DIR=doc/arch` 已写死在 hook 里。如果用户指定了其他路径,当前版本要求用户手动在 hook 顶部设置 `ADR_DIR=...`(或 export 环境变量)。v0.2 会提供 `.git/hooks/spec-vc.env` 自动加载机制。
...
下一步:
  1. 创建首条实际 ADR:/spec-vc adr-new "<title>"
  2. 正常开发 → git commit,subject 行末尾加 [ADR-NNN] 或 [ADR-none]
  3. 周期性运行 /spec-vc adr-status 检查锚定健康
```
