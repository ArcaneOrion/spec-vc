...
ADR-009 实施完成：移除 hash chain token，引入 PostToolUse hook (Agent matcher) 记录 subagent 调用到 subagent-sessions.log，commit-msg hook 校验链升级为 token→subagent session→ADR。修复 3 个 bug：cmd_hook_post_tool_use 重复定义、_init_claude_hook 只匹配 Bash、旧 stdin-JSON 测试过时。107 pytest 全通过。
