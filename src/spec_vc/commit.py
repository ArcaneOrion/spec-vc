...
def prepare_audit_prompt(ctx: CommitContext) -> str:
    ...
    lines.append("## Git Diff (staged changes)")
    lines.append("")
    lines.append("```diff")
    ...
    lines.append("- Warnings: N")
    lines.append("- Failed: N")
    lines.append("- Verdict: PASSED | BLOCKED")
    lines.append("```")
...
def prepare_test_prompt(ctx: CommitContext) -> str:
    ...
    lines.append("- 执行结果: PASSED / FAILED")
    lines.append("- 覆盖项: 列出覆盖的 Spec 条目")
    lines.append("")
    lines.append("## Summary")
    lines.append("- Total tests: N")
    lines.append("- Passed: N")
    lines.append("- Failed: N")
    lines.append("- Verdict: PASSED | FAILED")
    lines.append("```")
